...
if __name__ == "__main__":
    ...
    def _register_interrupt_saver(get_data_fn, filename: str = "episode_dataNew.json ") -> None:
    ...
    for episode in range(1, args.test_episodes + 1):
        ...
        for i in range(num_iterations):
            good_action = False
            #is_json_ok = True
            result = llm_query.get_action_from_obs_react(observation, memories)
            
            # Handle both 3-value and 4-value returns (with or without tokens_used)
            if len(result) == 4:
                is_valid, response_dict, action, tokens_used = result
            else:
            ...
            if args.max_tokens_limit > 0 and total_tokens_used > args.max_tokens_limit:
                ...
                try:
                    with open("episode_dataNew.json ", "w") as json_file:
